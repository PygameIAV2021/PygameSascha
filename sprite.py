...
class Player(pg.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.damaged:
            try:
                self.image.fill((255, 0, 0, next(self.damaged_alpha)), special_flags=pg.BLEND_RGB_MULT)
            except:
                self.damaged = False
        self.rect = self.image.get_rect()
...
class Bullet(pg.sprite.Sprite):
    ...
    def update(self):
        # self.rot = self.game.player.pos.angle_to(vec(1, 0))
...
class ExploFlash(pg.sprite.Sprite):
    def __init__(self, game, pos):
        ...
        self.game = game
        size = randint(20, 50)
        self.image = pg.transform.scale(choice(game.explo), (size, size))
        self.rect = self.image.get_rect()
        self.pos = pos
        self.rect.center = pos
        self.spawn_time = pg.time.get_ticks()
